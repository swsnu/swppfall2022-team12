...
class ReviewViewSet(viewsets.GenericViewSet, generics.RetrieveDestroyAPIView):
    """
    ...
    def create(self, request):
        """Create Review"""
        data = request.data
        context = {"course": data.get("course"), "author": request.user}
    ...
    def list(self, request):
        ...
        if not course_id:
            raise FieldError("missing fields [course]")
        course = get_object_or_404(Course, id=course_id)
        reviews = course.reviews.all().order_by(F("created_at").desc())
        if f_param:
            if f_param == "likes":
                reviews = reviews.order_by(F("likes").desc())
            elif f_param == "time_asc":
                reviews = reviews.order_by(F("created_at").asc())
            elif f_param == "time_desc":
                reviews = reviews.order_by(F("created_at").desc())
            elif f_param == "rate_asc":
                reviews = reviews.order_by(F("rate").asc())
            elif f_param == "rate_desc":
                reviews = reviews.order_by(F("rate").desc())

        reviews = Paginator(reviews, 20).get_page(page)
        return Response(
            self.get_serializer(reviews, many=True).data, status=status.HTTP_200_OK
        )

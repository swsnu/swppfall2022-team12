...
class UserTestCase(TestCase):
    ...
    def test_signup(self):
        ...
        response = self.client.post(
            "/api/user/signup/", data=signup_data, content_type="application/json"
        )
    ...
    def test_login(self):
        ...
        Login test case.
        """
        login_data = {"email": self.user.email, "password": "12345678"}
        response = self.client.put(
            "/api/user/login/", data=login_data, content_type="application/json"
        )
        ...
        self.assertIn("refresh", data["token"].keys())
        self.assertSetEqual(
            set(data["tags"]), set(self.user.tags.values_list("id", flat=True))
        )

        login_data = {"email": "wrongemail@test.com", "password": "12345678"}
        response = self.client.put(
            "/api/user/login/", data=login_data, content_type="application/json"
        )
    ...
    def test_logout(self):
        ...
        response = self.client.get("/api/user/logout/", content_type="application/json")

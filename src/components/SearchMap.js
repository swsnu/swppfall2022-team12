...
function SearchMap(keyword) {
  ...
  useEffect(() => {
    ...
    if (keyword.keyword !== "") {
      ps.keywordSearch(keyword.keyword, (data, status, _pagination) => {
    ...
    <>
      <div style={{ width: '30%', float:'right' }}>
        ...
        <div className="selection">
          ...
          {selected && selected.map(location => (
            <li key={`marker-${location.content}-${location.position.lat},${location.position.lng}`}>
              {location.content}
            </li>
          ))}
        </div>

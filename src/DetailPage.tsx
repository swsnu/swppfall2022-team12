    ...
    const onPlay = () => {
        ...
        for (let index = 0; index < points.length; index++) {
            ...
            if(index==points.length-1){
                const a = ("dlat="+element.latitude+"&dlng="+element.longitude+"&dname="+encodeURI("임시한국어"));
                tempArray.splice(1,0,a);
            }else{
                tempArray.push("&v"+(index+1)+"lat="+element.latitude+"&v"+(index+1)+"lng="+element.longitude+"&v"+(index+1)+"name="+encodeURI("임시한국어2") )
            }
        }
